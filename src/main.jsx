...
function App() {
  ...
  return <div className="app-shell">
    <header className="app-header"><a className="brand" href="/" aria-label="Queryroom home"><span className="brand-mark"><Braces size={21}/></span><span>queryroom<span className="brand-period">.</span></span></a><div className="header-divider"/><span className="workspace-label">Your SQL practice space</span><div className="header-right"><span className="local-indicator"><span/> {hosted?'Shared workspace':'Local workspace'}</span><IconButton label={theme === 'light' ? 'Switch to dark theme' : 'Switch to light theme'} onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}>{theme === 'light' ? <Moon size={17}/> : <Sun size={18}/>}</IconButton><IconButton label="Help and keyboard shortcuts" onClick={() => setModal('help')}><CircleHelp size={18}/></IconButton><span className="avatar" title="Your personal workspace">Y</span></div></header>
    ...
    <footer className="app-footer"><span><span className="footer-dot"/> {hosted?'Shared SQL workspace':'All systems local'} <span className="footer-separator">·</span> {hosted?'Progress saved to your workspace':'Your progress stays on this device'}</span><span>Made for your next “aha.” <Sparkles size={12}/></span></footer>
