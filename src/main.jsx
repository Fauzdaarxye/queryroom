...
async function api(path, options) {
  ...
  if (!response.ok) throw new Error(data.error || 'Could not connect to the workspace server.');
  return data;
}
...
function App() {
  ...
  const persistDraft = (key, draft, draftNotes) => {
    ...
    return progressStore.saveDraft(key, draft, draftNotes).then(saved => {
      ...
      if (activeSlug.current === key && draftValues.current[key] === snapshot) setSaveStatus('Saved in this browser');
    }).catch(() => {
      if (activeSlug.current === key && draftValues.current[key] === snapshot) setSaveStatus('Not saved · browser storage unavailable');
    });
  };
  ...
  useEffect(() => {
    Promise.all([api('/api/problems'), api('/api/engines')]).then(async ([catalog, available]) => {
      // Support an already-running server during frontend upgrades.
      const p=catalog.map(item=>({...item,id:item.id || `qr${item.number}`}));
      const saved = await progressStore.read(p.map(item => item.slug));
      ...
      const first=p.find(item=>item.slug===requested || item.id===requested.trim().toLowerCase())||p[0];
  ...
  useEffect(() => {
    ...
    if (query===pending?.draft && notes===pending?.notes) {
      if (query===saved?.draft && notes===saved?.notes) setSaveStatus('Saved in this browser');
      return;
    }
  ...
  const run = useCallback(async mode => {
    ...
    try {
      ...
      if (data.submission) {
        try {
          const saved = await progressStore.recordSubmission(slug, data.submission);
          setProgress(prev => ({ ...prev, [slug]: saved }));
        } catch {
          setSaveStatus('Not saved · browser storage unavailable');
          setToast('Your query was checked, but this browser could not save the submission. Allow site storage or free some space, then submit again.');
          return;
        }
      }
  ...
  const onBookmark = async () => {
    try {
      const saved = await progressStore.toggleBookmark(slug);
      setProgress(prev => ({ ...prev, [slug]: saved }));
    } catch { setToast('Could not save your bookmark in this browser. Allow site storage or free some space, then try again.'); }
  };
  ...
  return <div className="app-shell">
    <header className="app-header"><a className="brand" href="/" aria-label="Queryroom home"><span className="brand-mark"><Braces size={21}/></span><span>queryroom<span className="brand-period">.</span></span></a><div className="header-divider"/><span className="workspace-label">Your SQL practice space</span><div className="header-right"><span className="local-indicator"><span/> Personal progress</span><IconButton label={theme === 'light' ? 'Switch to dark theme' : 'Switch to light theme'} onClick={() => setTheme(theme === 'light' ? 'dark' : 'light')}>{theme === 'light' ? <Moon size={17}/> : <Sun size={18}/>}</IconButton><IconButton label="Help and keyboard shortcuts" onClick={() => setModal('help')}><CircleHelp size={18}/></IconButton><span className="avatar" title="Your personal workspace">Y</span></div></header>
